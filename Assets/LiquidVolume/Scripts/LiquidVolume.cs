...
using LiquidVolumeFX.MIConvexHull;//확인 필요.
using System.Linq;//확인 필요.
using Random = UnityEngine.Random;//와 이건 뭐지?
...
namespace LiquidVolumeFX {
    ...
    public partial class LiquidVolume : MonoBehaviour {
        ...
        public Vector3 spillPos;//추가한 내용.
        ...
        public bool GetSpillPoint(out Vector3 spillPosition, out float spillAmount, float apertureStart = 1f) {
            ...
            for (int k = 0; k < verticesSorted.Length; k++)
            {
                ...
                if (vertex.y < clampy)
                    break;
                vertex = transform.TransformPoint(vertex);//버텍스의 로컬좌표계를 월드 좌표계로.
            ...
            if (!crossed)//팁에 교차하지 않으면
                return false;//팁에 닿지 않았다.

            spillPosition = vt;
            spillPos = spillPosition;
            Debug.Log("SpillPos : "+ spillPosition);
